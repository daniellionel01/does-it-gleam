...
-spec be_ok({ok, DRF} | {error, any()}) -> DRF.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, DRK}) -> DRK.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(DRN)) -> DRN.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
