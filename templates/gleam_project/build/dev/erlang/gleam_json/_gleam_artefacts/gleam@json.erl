...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(DLH)) -> {ok, DLH} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(DLB)) -> {ok, DLB} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(DKX)) -> {ok, DKX} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(DLN), fun((DLN) -> json())) -> json().
...
-spec array(list(DLR), fun((DLR) -> json())) -> json().
